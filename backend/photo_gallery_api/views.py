...
from .serializers import TodoSerializer, UserSerializer, PhotoSerializer
...
class PhotoView(viewsets.ModelViewSet):
    ...
    def list(self, request):
        print('list photos')
        queryset = Photo.objects.all().filter(user=request.user.id)
        return Response(PhotoSerializer(queryset, many=True, context={ 'request': request }).data, status=200)

    def retrieve(self, request, pk=None):
        print('retrieve photo')
        queryset = Photo.objects.all().filter(user=request.user.id)
        photo = get_object_or_404(queryset, pk=pk)
        return Response(PhotoSerializer(photo, many=False, context={ 'request': request }).data, status=200)
